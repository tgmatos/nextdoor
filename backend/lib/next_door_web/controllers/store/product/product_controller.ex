defmodule NextDoorWeb.ProductController do
  ...
  def create(conn, %{"product" => %{"name" => name, "description" => description, "price" => price, "stock" => quantity}}) do
    %{"sub" => owner_id} = Guardian.Plug.current_claims(conn)
    with {:ok, product} <- Products.create(owner_id, %{name: name, description: description, price: price, quantity: quantity}) do
